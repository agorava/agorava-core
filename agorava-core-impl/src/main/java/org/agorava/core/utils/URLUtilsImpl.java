...
public class URLUtilsImpl implements URLUtils {
    ...
    @Override
    public String buildUri(String url, Map<String, ?> parameters) {
        String queryString = formURLEncodeMap(parameters);
    ...
    public String buildUri(String url, Object pojo) {
        ...
        String queryString = mapToQueryString(pojoMap);
        if (queryString.equals(EMPTY_STRING)) {
            return url;
        } else {
            url += url.indexOf(QUERY_STRING_SEPARATOR) != -1 ? PARAM_SEPARATOR : QUERY_STRING_SEPARATOR;
