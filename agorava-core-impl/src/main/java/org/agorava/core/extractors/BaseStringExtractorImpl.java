...
import org.agorava.core.rest.ParameterListImpl;

/**
 * Default implementation of {@link org.agorava.core.api.extractors.BaseStringExtractor}. Conforms to OAuth 1.0a
 *
...
public class BaseStringExtractorImpl implements BaseStringExtractor {
    ...
    private String getSortedAndEncodedParams(OAuthRequest request) {
        ParameterList params = new ParameterListImpl();
        params.addAll(request.getQueryStringParams());
        params.addAll(request.getBodyParams());
        params.addAll(new ParameterListImpl(request.getOauthParameters()));
